...
class CentralVCriticPDNSContinuous(nn.Module):
    def __init__(self, scheme, args):
        super(CentralVCriticPDNSContinuous, self).__init__()
        ...
        self.critics = [MLP(input_shape, args.hidden_dim, self.n_outputs) for _ in range(self.n_agents)]
    ...
    def _build_inputs(self, batch, t=None):
        ...
        if self.args.obs_last_action:
            # last actions
            if t == 0:
                inputs.append(th.zeros_like(batch["actions"][:, 0:1]).view(bs, max_t, 1, -1))
            elif isinstance(t, int):
                inputs.append(batch["actions"][:, slice(t-1, t)].view(bs, max_t, 1, -1))
            else:
                last_actions = th.cat([th.zeros_like(batch["actions"][:, 0:1]), batch["actions"][:, :-1]], dim=1)
