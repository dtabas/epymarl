...
class ActorCriticPDLearnerContinuous:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        avail_actions = batch["avail_actions"][:,0] # Does not accommodate changing action spaces
        low,high = th.split(avail_actions, 1, dim=-1)
        low = th.squeeze(low,-1)
        high = th.squeeze(high,-1)
        normalized_actions = (actions - (high+low)/2)/((high-low)/2)
        ...
        sigma = self.mac.action_selector.schedule.eval(t_env)
        log_pi_taken = self.normalized_action_pdf(actions,pi,sigma)
        ...
        pg_loss = -((advantages * log_pi_taken) * mask_).sum() / mask_.sum()
    ...
    def normalized_action_pdf(self,y, mu, sigma):
                
        assert np.shape(mu) == np.shape(y)
        ...
        # Calculate the pdf of Y and adjust for the change of variables
        
        #return norm.pdf(x, loc=mu, scale=sigma) * np.abs(1 / (1 - y**2))
        log_prob_marginal = normal_dist.log_prob(x) + th.log(th.abs(1/(1-y**2)) + 1e-10)
        log_prob_joint = th.sum(log_prob_marginal,dim=-1)
